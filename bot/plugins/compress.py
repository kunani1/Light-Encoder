...
async def renew(e):
    await e.reply_text("🗑️ **Cleared Queued, Working Files and Cached Downloads!**", quote=True)
    list_handler.clear()
    data.clear()
    queue.delete_many({})
...
async def mediainfo(app, message):
  if message.reply_to_message:
   ...
   await app.send_message(message.chat.id, "💡 **Reply To Video File.**", reply_to_message_id=message.id)
